...
async function obtenerDashboardHomeService({ mes, anio, vendedor }) {
  ...
  const rows = vStr ? base.filter((r) => sameText(r.vendedor, vStr)) : base;

  const totalMes = rows.reduce((acc, r) => acc + toNumber(r.total, 0), 0);
  ...
  const ultimasVentas = rows.slice(0, 5).map((r) => ({
    id: r.id,
