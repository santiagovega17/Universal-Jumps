...
const { normalizePais, normalizeFormaPago } = require('./cajaService');
...
async function insertarIngresoCajaDesdeVenta(venta) {
  ...
  const payload = {
    ...
    forma_pago: normalizeFormaPago(venta.formaPago),
...
async function guardarVentaService(datos) {
  ...
  const payload = {
    ...
    forma_pago: normalizeFormaPago(datos.formaPago),
    cantidad,
    precio,
    total,
...
function toSerieDiaria({ y, m, lastDay, rows }) {
  ...
  rows.forEach((r) => {
    ...
    if (!k) return;
    byDay[k] = (byDay[k] || 0) + getVentaMontoArs(r);
  });
  const serieDiaria = [];
...
function getVentaMontoArs(row) {
  const totalArs = toNumber(row.total_ars, 0);
  if (totalArs > 0) return totalArs;
  const moneda = normalizeMonedaCode(row.moneda_codigo || '');
  const total = toNumber(row.total_moneda != null && row.total_moneda !== '' ? row.total_moneda : row.total, 0);
  if (!moneda || moneda === 'ARS') return total;
  const cotizacion = toNumber(row.cotizacion_moneda, 0) || toNumber(row.cotizacion, 0) || 1;
...
async function obtenerDashboardHomeService({ mes, anio, vendedor }) {
  ...
  const { data, error } = await supabase
    .from('venta')
    .select('id, vendedor, fecha, total, total_moneda, total_ars, moneda_codigo, cotizacion, cotizacion_moneda, cliente, estado, forma_pago, created_at, anio, pais')
  ...
  if (mesVistaEsMesCalendarioActual && hoyDentroDelMesVista) {
    ...
    totalMes = scoped.reduce((acc, r) => {
      const f = fechaIsoCorta(r);
      return f >= cur.start && f <= curHasta ? acc + getVentaMontoArs(r) : acc;
    }, 0);
    ...
    totalMesPrev = scoped.reduce((acc, r) => {
      const f = fechaIsoCorta(r);
      return f >= prevRange.start && f <= prevHasta ? acc + getVentaMontoArs(r) : acc;
    }, 0);
  } else {
    totalMes = curRows.reduce((acc, r) => acc + getVentaMontoArs(r), 0);
    totalMesPrev = prevRows.reduce((acc, r) => acc + getVentaMontoArs(r), 0);
  }
  ...
  const totalAnualYtd = scoped.reduce((acc, r) => {
    ...
    return acc + getVentaMontoArs(r);
  }, 0);
  const totalHoy = scoped.reduce((acc, r) => {
    const f = fechaIsoCorta(r);
    return f === hoyIso ? acc + getVentaMontoArs(r) : acc;
  }, 0);
  const totalAyer = scoped.reduce((acc, r) => {
    const f = fechaIsoCorta(r);
    return f === ayerIso ? acc + getVentaMontoArs(r) : acc;
  }, 0);
  ...
  if (!vStr) {
    ...
    curRows.forEach((r) => {
      ...
      rankingMap[name] = (rankingMap[name] || 0) + getVentaMontoArs(r);
    });
  ...
  curRows.forEach((r) => {
    ...
    porPaisMap[p] = (porPaisMap[p] || 0) + getVentaMontoArs(r);
  });
...
async function borrarVentaService(datos) {
  ...
  if (!ingErr && ingresos && ingresos.length > 0) {
    const target = ingresos.find((row) => {
      ...
      const samePago = normalizeFormaPago(row.forma_pago) === normalizeFormaPago(venta.forma_pago);
...
async function confirmarVentaConStockService(datos) {
  const supabase = getSupabaseAdmin();
  const cotizaciones = await getCotizacionesMonedaMap(supabase);
  const monedasConfig = await getMonedasPaisConfig(supabase);
  ...
  const vendedor = normalizeText(datos.vendedor);
  if (!vendedor) throw new Error('Vendedor es requerido');
  ...
  if (items.length === 0) throw new Error('Debe enviar al menos un item');

  const usuarioId = await getUsuarioIdByNombre(vendedor);

  const itemsPayload = items.map((it) => {
    const productoId = normalizeText(it.producto_id || it.productoId);
    if (!productoId) throw new Error('Cada item debe tener producto_id');

    return {
      producto_id: productoId,
      talle: normalizeText(it.talle || 'UNICO') || 'UNICO',
      cantidad: toNumber(it.cantidad, 0),
      precio_unitario: toNumber(it.precio_unitario != null ? it.precio_unitario : it.precio, 0),
      porcentaje_descuento: toNumber(it.porcentaje_descuento, 0),
      descuento_monto: toNumber(it.descuento_monto, 0),
      alicuota_iva: toNumber(it.alicuota_iva, 21),
      precio_fuente_id: normalizeText(it.precio_fuente_id || ''),
    };
  });

  const { data, error } = await supabase.rpc('fn_confirmar_venta_con_stock', {
    p_usuario_id: usuarioId,
    p_vendedor: vendedor,
    p_fecha: fecha.toISOString().slice(0, 10),
    p_pais: pais,
    p_cliente_id: datos.clienteId || datos.cliente_id || null,
    p_cliente: normalizeText(datos.cliente),
    p_forma_pago: normalizeFormaPago(datos.formaPago),
    p_moneda_codigo: monedaCodigo,
    p_cotizacion_moneda: cotizacionMoneda,
    p_observaciones: normalizeText(datos.comentario),
  ...
  return {
    error: false,
    mensaje: 'Venta confirmada con stock correctamente.',
    data,
  };
}
