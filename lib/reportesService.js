...
function buildMesesResumenVendedor({ year, vendedor, ventasRows, objetivosIndex, cotizaciones }) {
  ...
  meses.forEach((itemMes) => {
    ...
    const calc = calcularRangoPorcentajeMes(itemMes.totalFacturado, objetivosMes);
    const objetivoBase = calc.objetivoBase;
    const objetivoRango = calc.targetObj;
    const porcentajeBotas = objetivoRango ? toNumber(objetivoRango.pctBotas, 0) : 0;
    const porcentajeCerts = objetivoRango ? toNumber(objetivoRango.pctCerts, 0) : 0;
