...
function calcularRangoPorcentajeMes(montoMes, objetivosMes) {
  ...
  if (sorted.length === 0) {
    return { rango: null, porcentaje: 0 };
  }
  ...
  const targetObj = rangoMes !== null ? sorted.find((o) => toNumber(o.rango, 0) === rangoMes) : null;
...
async function obtenerTodasLasComisionesService() {
  ...
  function ventaCuentaParaRangoObjetivo(conceptoRaw) {
    ...
    return enBotas || enCerts;
  }

  const objetivosRows = await getObjetivosRows();
  /** Clave `${anio}_${mes}` → vendedor (upper) → lista de rangos (incluye año anterior para fallback mes 1 → dic) */
  const objetivos = {};
  ...
  function objetivosParaVendedorYMesConFallback(vendorKey, mesNum) {
    let L = objetivosParaVendedorYMes(vendorKey, mesNum);
    ...
    const mesKeyPrev = `${py}_${pm}`;
    const mapPrev = objetivos[mesKeyPrev] || {};
    if (mapPrev[vendorKey] && mapPrev[vendorKey].length) return mapPrev[vendorKey];
    return mapPrev.__DEFAULT__ || [];
  }
  ...
  (ventasRows || []).forEach((row) => {
    const vendedor = normalizeText(row.vendedor);
    if (!vendedor) return;
    if (!ventaCuentaParaRangoObjetivo(row.concepto)) return;
  ...
  TRIMESTRES.forEach((trim) => {
    ...
    todosVendedores.forEach((v) => {
      ...
      const mes1Calc = calcularRangoPorcentajeMes(mes1, objetivosParaVendedorYMesConFallback(key, mesNum1));
      const mes2Calc = calcularRangoPorcentajeMes(mes2, objetivosParaVendedorYMesConFallback(key, mesNum2));
      const mes3Calc = calcularRangoPorcentajeMes(mes3, objetivosParaVendedorYMesConFallback(key, mesNum3));
