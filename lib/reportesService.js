...
function getMonedaPorPais(pais, monedasConfig) {
  return normalizeMonedaCode(getMonedaDefaultPorPais(pais || '', monedasConfig));
}
...
function getVentaMontoOriginal(row) {
  return toNumber(row.total_moneda != null && row.total_moneda !== '' ? row.total_moneda : row.total, 0);
}

function getVentaMontoArs(row, pais, cotizaciones, monedasConfig) {
  const totalArs = toNumber(row.total_ars, 0);
  if (totalArs > 0) return totalArs;
  const moneda = normalizeMonedaCode(row.moneda_codigo || getMonedaPorPais(pais, monedasConfig));
  const montoOriginal = getVentaMontoOriginal(row);
  if (moneda === 'ARS') return montoOriginal;
  const cotizacionUsada = toNumber(row.cotizacion_moneda, 0) || toNumber(row.cotizacion, 0)
    || getCotizacionParaPaisYMoneda({ pais, moneda, cotizaciones, monedasConfig });
  return montoOriginal * cotizacionUsada;
}
...
function buildMesesResumenVendedor({ year, vendedor, ventasRows, objetivosIndex, cotizaciones, monedasConfig }) {
  ...
  (ventasRows || []).forEach((row) => {
    ...
    const montoOriginal = getVentaMontoOriginal(row);
    const monedaOriginal = normalizeMonedaCode(row.moneda_codigo || getMonedaPorPais(pais, monedasConfig));
    const cotizacionUsada = toNumber(row.cotizacion_moneda, 0) || toNumber(row.cotizacion, 0)
      || getCotizacionParaPaisYMoneda({ pais, moneda: monedaOriginal, cotizaciones, monedasConfig });
    const montoArs = getVentaMontoArs(row, pais, cotizaciones, monedasConfig);
    const montoArsSinIva = montoArs / getIvaDivisorByPais(pais);
    const itemMes = meses[monthIndex];

    itemMes.totalFacturado += montoArs;
    itemMes.cantidadVentas += 1;
    if (grupo === 'BOTAS') {
      itemMes.totalBotas += montoArs;
      itemMes.totalBotasSinIva += montoArsSinIva;
    }
    if (grupo === 'CERTS') {
      itemMes.totalCerts += montoArs;
      itemMes.totalCertsSinIva += montoArsSinIva;
    }
  ...
  meses.forEach((itemMes) => {
    ...
    itemMes.comisionBotas = itemMes.totalBotasSinIva * (porcentajeBotas / 100);
    itemMes.comisionCerts = itemMes.totalCertsSinIva * (porcentajeCerts / 100);
...
async function obtenerReporteConsolidadoService(anio) {
  ...
  (data || []).forEach((row) => {
    ...
    const pais = normalizePais(row.pais || '');
    const totalArs = getVentaMontoArs(row, pais, cotizaciones, monedasConfig);
...
async function obtenerTodasLasComisionesService(anio) {
  ...
  const { data: ventasRows, error: ventasErr } = await supabase
    .from('venta')
    .select('vendedor, fecha, pais, total, total_moneda, total_ars, moneda_codigo, cotizacion, cotizacion_moneda, anio, concepto')
    .eq('anio', year);
...
async function obtenerComisionesVendedorService(vendedor, anio, mes, trimestre) {
  ...
  const { data: ventasRows, error: ventasErr } = await supabase
    .from('venta')
    .select('id, vendedor, fecha, pais, cliente, concepto, forma_pago, cantidad, total, total_moneda, total_ars, moneda_codigo, observaciones, cotizacion, cotizacion_moneda, anio, created_at')
    .eq('anio', year);
  if (ventasErr) throw new Error(ventasErr.message);
  ...
  const resumenMeses = mesesFiltrados.map((itemMes) => ({
    ...
    totalBotasSinIva: itemMes.totalBotasSinIva,
    totalCertsSinIva: itemMes.totalCertsSinIva,
  ...
  const resumen = resumenMeses.reduce(
    (acc, itemMes) => {
      ...
      acc.totalBotasSinIva += itemMes.totalBotasSinIva;
      acc.totalCertsSinIva += itemMes.totalCertsSinIva;
