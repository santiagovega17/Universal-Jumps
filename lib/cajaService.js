...
async function getBalanceCaja({ pais, mes, anio }) {
  ...
  return {
    ...
    ingresosMonedaReferencia: ingresos / cotRef,
    egresosMonedaReferencia: egresos / cotRef,
    pendientesMonedaReferencia: pendientes / cotRef,
    saldoMonedaReferencia: saldo / cotRef,
  };
}
...
async function getMediosPagoCaja({ pais, mes, anio }) {
  ...
  (ventas || []).forEach((v) => {
    ...
    const montoArs = getVentaMontoArs(v, paisNormalizado, cotizaciones, monedasConfig);
    if (!mediosPago[medio]) mediosPago[medio] = { ingresos: 0, egresos: 0, saldo: 0 };
