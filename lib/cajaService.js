...
/** Límites inclusive en `YYYY-MM-DD` para filtrar historial de caja en la base (evita el tope típico de 1000 filas al traer todo el país). */
function getHistorialCajaFechaBounds(mes, anio) {
  const y = Number(anio);
  const year = Number.isFinite(y) && y > 0 ? y : new Date().getUTCFullYear();
  const mesNum = Number(mes);
  if (!mesNum || mesNum === 0) {
    return { desde: `${year}-01-01`, hasta: `${year}-12-31` };
  }
  const m = Math.min(12, Math.max(1, mesNum));
  const mm = String(m).padStart(2, '0');
  const lastDay = new Date(Date.UTC(year, m, 0)).getUTCDate();
  const dd = String(lastDay).padStart(2, '0');
  return { desde: `${year}-${mm}-01`, hasta: `${year}-${mm}-${dd}` };
}
...
async function getHistorialCaja({ pais, mes, anio }) {
  const supabase = getSupabaseAdmin();
  const paisNormalizado = normalizePais(pais);
  const { desde, hasta } = getHistorialCajaFechaBounds(mes, anio);
  const { data, error } = await supabase
    .from('cajaMovimiento')
