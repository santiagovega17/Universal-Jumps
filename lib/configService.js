...
async function agregarNuevoConceptoDescripcionAMovimientos(tipo, nombre, padre, pais) {
  const supabase = getSupabaseAdmin();
  const hoy = new Date();
  const mesInicio = hoy.getMonth() + 1;
  const paisNorm = normalizePais(pais || 'ARGENTINA');
  const concepto = tipo === 'CONCEPTO' ? normalizeText(nombre) : normalizeText(padre || '');
  const descripcion = tipo === 'DESCRIPCION' ? normalizeText(nombre) : '';
  if (!concepto) return;

  const { data, error } = await supabase
    .from('cajaMovimiento')
    .select('fecha, tipo, concepto, descripcion, pais')
    .eq('tipo', 'EGRESO')
  ...
  (data || []).forEach((row) => {
    const dt = row.fecha ? new Date(row.fecha) : null;
    if (!dt || Number.isNaN(dt.getTime())) return;
    const year = dt.getUTCFullYear();
    const month = dt.getUTCMonth() + 1;
    if (year === ANIO_PROYECTADO) existentes.add(month);
  });
  ...
  if (rows.length > 0) {
    const { error: insertError } = await supabase.from('cajaMovimiento').insert(rows);
...
async function guardarItemConfigService(tipo, nombre, padre, pais, sentido) {
  ...
  const esConceptoEgreso = tipoNorm === 'CONCEPTO' && sentidoNorm === 'EGRESO';
  const esDescripcion = tipoNorm === 'DESCRIPCION' && !!padreNorm;
  if (esConceptoEgreso || esDescripcion) {
    await agregarNuevoConceptoDescripcionAMovimientos(tipoNorm, nombreNorm, padreNorm, paisNorm);
  }
  return { error: false };
}
...
async function actualizarCotizacionesDesdeExternas() {
  ...
  const supabase = getSupabaseAdmin();
  for (const [pais, factor] of Object.entries(factores)) {
    if (!pais || !Number.isFinite(factor) || factor <= 0) continue;
    await supabase.from('configCotizacion').upsert({ pais, factor }, { onConflict: 'pais' });
  }
  return { error: false, actualizados: Object.keys(factores).length };
}
